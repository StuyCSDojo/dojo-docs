...
^^^^^^^^^^^^^^^^^^^^^^^^^^^
The ``route()`` decorator is used to bind URL to a function.  The index page for your domain is refer to as ``/``.  The syntax is:

::

   @app.route("<path/to/page>")

To bound a function to the root route, you would write:

::

   @app.route("/")                 # refers to root
   def hello():                    # the function name does not matter
       return "Hello World"

The problem here is that the chunk of code must resides in the same file that you declare:

::

   app = Flask(__name__)

This is problematic for large projects as it prevents you from using modular design...
